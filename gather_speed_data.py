...
if __name__ == "__main__":
    with (open('prey-speed-DRTest6.csv', 'w', newline='') as csvfile):
        ...
        permutations = []
        for M in [2, 3, 4, 5]:
            for d, e in [(0.5, 0.4), (0.5, 0.5), (0.5, 0.6), (0.5, 0.7), (0.5, 0.8), (0.5, 0.9), (0.5, 1.0),
                         (0.5, 1.1)]:
        ...
        for i, p in enumerate(permutations):
            ...
            colors = np.arange(len(vel))
            fig, ax = plt.subplots(figsize=(9, 6))
            scatter = ax.scatter(vel, ang, c=colors, cmap='rainbow')

            # Set title and colorbar
            plt.title("         Speed Behaviors", fontsize=16, y=1.05)
            plt.suptitle(f"M{M}_c{c}_d{d}_e{e}       ", fontsize=12, y=0.92)
            ...
            plt.savefig(f"SpeedGraphsFinal/M{M}_c{c}_d{d}_e{e}.png", dpi=200)
            # plt.show()
